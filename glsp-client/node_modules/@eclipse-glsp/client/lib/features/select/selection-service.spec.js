...
function createContainer() {
    ...
    container.rebind(types_1.TYPES.IModelFactory).to(sprotty_1.SGraphFactory).inSingletonScope();
    container.bind(types_1.TYPES.IFeedbackActionDispatcher).to(MockFeedbackActionDispatcher).inSingletonScope();
    container.bind(selection_service_1.SelectionService).toSelf().inSingletonScope();
    container.bind(types_1.TYPES.SelectionService).toService(selection_service_1.SelectionService);
    return container;
}
describe('SelectionService', () => {
    ...
    beforeEach(() => {
        ...
        // eslint-disable-next-line deprecation/deprecation
        graphFactory = container.get(types_1.TYPES.IModelFactory);
        root = createRoot('node1', 'node2', 'node3', 'node4', 'node5');
        selectionService = container.get(types_1.TYPES.SelectionService);
        feedbackDispatcher = container.get(types_1.TYPES.IFeedbackActionDispatcher);
    });
    ...
    describe('Listeners', () => {
        ...
        it('A registered listener should receive the selected elements in the right order.', () => {
            ...
            assertListener(listener, undefined, [], 0);
            selectionService.updateSelection(root, ['node2', 'node1'], []);
            (0, chai_1.expect)(() => assertListener(listener, root, ['node1', 'node2'], 1)).to.throw(chai_1.AssertionError, 'ordered members');
        });
    ...
    function assertListener(listener, expectedRoot, expectedSelection, expectedCalled) {
        (0, chai_1.expect)(listener.getRoot()).to.equal(expectedRoot);
        (0, chai_1.expect)(listener.getSelectedElements()).to.have.lengthOf(expectedSelection.length);
        (0, chai_1.expect)(listener.getSelectedElements()).to.have.ordered.members(expectedSelection);
        (0, chai_1.expect)(listener.getCalled()).to.equal(expectedCalled);
    }
});
